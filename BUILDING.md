...
```powershell
docker run --rm -v "${PWD}:/workspace" ps2max/dev:v20260228 sh -c 'set -eu
if [ ! -e "$PS2SDK/ps2dev.cmake" ]; then ln -s ../share/ps2dev.cmake "$PS2SDK/ps2dev.cmake"; fi
sh /workspace/tools/bootstrap-ps2-libtiff.sh
cmake -S /workspace/nhddl -B /workspace/nhddl/build-luna -DCMAKE_BUILD_TYPE=Release -DLUNA_GLASS_UI=ON -DLUNA_EMULATOR_BUILD=OFF
cmake --build /workspace/nhddl/build-luna --parallel 2'
```

Output: `nhddl/build-luna/luna.elf`

The Release Candidate, CI artifact, and emulator validation build all enable
`LUNA_GLASS_UI`. Keep this flag aligned so the packaged hardware ELF uses the
same glass UI that receives emulator validation. `LUNA_EMULATOR_BUILD` is kept
off for hardware and enables only PCSX2 direct-ELF path handling in the emulator
validation build.
...
The FMCB package reuses that promoted ELF and the matching Neutrino build. It
does not move artwork or writable library state onto the memory card. Build and
